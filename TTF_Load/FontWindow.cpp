...
void FontWindow::CalculateLayout()
{
	if (pRenderTarget != NULL)
	{
		...
		MoveWindow(hwndFileFontSize, 420, 50, 50, 30, TRUE);
		MoveWindow(hwndParamFontSize, 420, 125, 50, 30, TRUE);
		MoveWindow(hwndHeaderFontSize, 420, 200, 50, 30, TRUE);
	}
}
...
void FontWindow::OnPaint()
{
	...
	if (SUCCEEDED(hr))
	{
		...
		pRenderTarget->DrawTextW(L"File names:", static_cast<UINT32>(wcslen(L"File names:")), pTextFormat_files, textLayout, pBrush);

		textLayout = D2D1::RectF(10, 115, 110, 165);
		pRenderTarget->DrawTextW(L"Parameters:", static_cast<UINT32>(wcslen(L"Parameters:")), pTextFormat_size, textLayout, pBrush);

		textLayout = D2D1::RectF(10, 185, 130, 235);
		pRenderTarget->DrawTextW(L"Headers:", static_cast<UINT32>(wcslen(L"Headers:")), pTextFormat_header, textLayout, pBrush);
...
void FontWindow::CreateDropLists() {
	hwndList_files = CreateWindow(WC_COMBOBOX, TEXT(""),
	...
	hwndList_sizes = CreateWindow(WC_COMBOBOX, TEXT(""),
		CBS_DROPDOWNLIST | CBS_HASSTRINGS | WS_CHILD | WS_OVERLAPPED | WS_VISIBLE | WS_VSCROLL,
		150, 125, 250, 300, m_hwnd, NULL, (HINSTANCE)GetModuleHandle(NULL),
		NULL);
	hwndList_headers = CreateWindow(WC_COMBOBOX, TEXT(""),
		CBS_DROPDOWNLIST | CBS_HASSTRINGS | WS_CHILD | WS_OVERLAPPED | WS_VISIBLE | WS_VSCROLL,
		150, 200, 250, 300, m_hwnd, NULL, (HINSTANCE)GetModuleHandle(NULL),
		NULL);
}
...
void FontWindow::CreateEditControl() {
	...
	hwndFileFontSize = CreateWindowExW(
		0,
		L"EDIT",
		NULL,
		WS_CHILD | WS_VISIBLE | ES_LEFT | ES_WANTRETURN | WS_BORDER,
	...
	hwndParamFontSize = CreateWindowExW(
		0,
		L"EDIT",
		NULL,
		WS_CHILD | WS_VISIBLE | ES_LEFT | ES_WANTRETURN | WS_BORDER,
...
void FontWindow::AddSystemFonts() {
	for (int i = 0; i < systemFonts.size(); i++) {
		SendMessage(hwndList_files, CB_ADDSTRING, 0, (LPARAM)systemFonts[i].c_str());
		SendMessage(hwndList_sizes, CB_ADDSTRING, 0, (LPARAM)systemFonts[i].c_str());
		SendMessage(hwndList_headers, CB_ADDSTRING, 0, (LPARAM)systemFonts[i].c_str());
	}
}
	...
	if (SUCCEEDED(hr)) {
		...
		if (SUCCEEDED(hr)) {
			...
			if (SUCCEEDED(hr)) {
				...
				SendMessage(hwndList_files, CB_ADDSTRING, 0, (LPARAM)wFontName.c_str());
				SendMessage(hwndList_sizes, CB_ADDSTRING, 0, (LPARAM)wFontName.c_str());
				SendMessage(hwndList_headers, CB_ADDSTRING, 0, (LPARAM)wFontName.c_str());
			}
		}
	}
}
...
LRESULT FontWindow::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND: {
		if (HIWORD(wParam) == BN_CLICKED) {
			if (LOWORD(wParam) == BTN_ADD) {
				...
				GetWindowTextW(hwndFileFontSize, NewFont, 256);
				structToSend->filesHeight = _wtoi(NewFont) / 0.75f;
				LRESULT ind = SendMessage(hwndList_files, CB_GETCURSEL, (WPARAM)0, (LPARAM)0);
				SendMessage(hwndList_files, CB_GETLBTEXT, (WPARAM)ind, (LPARAM)NewFont);
				wcscpy_s(structToSend->filesFont, NewFont);

				GetWindowTextW(hwndParamFontSize, NewFont, 256);
				structToSend->paramHeight = _wtoi(NewFont) / 0.75f;
				ind = SendMessage(hwndList_sizes, CB_GETCURSEL, (WPARAM)0, (LPARAM)0);
				SendMessage(hwndList_sizes, CB_GETLBTEXT, (WPARAM)ind, (LPARAM)NewFont);
				wcscpy_s(structToSend->paramFont, NewFont);

				GetWindowTextW(hwndHeaderFontSize, NewFont, 256);
				structToSend->headerHeight = _wtoi(NewFont) / 0.75f;
				ind = SendMessage(hwndList_headers, CB_GETCURSEL, (WPARAM)0, (LPARAM)0);
				SendMessage(hwndList_headers, CB_GETLBTEXT, (WPARAM)ind, (LPARAM)NewFont);
				wcscpy_s(structToSend->headersFont, NewFont);
	...
	case WM_SETDEF: {
		...
		SendMessage(hwndList_files, CB_SELECTSTRING, -1, (LPARAM)pData->filesFont);
		TCHAR buffer[256] = { 0 };
		errno_t result = _itow_s(static_cast<int>(round(pData->filesHeight * 0.75f)), buffer, 10);
		if (!result)
			SetWindowText(hwndFileFontSize, buffer);

		SendMessage(hwndList_sizes, CB_SELECTSTRING, -1, (LPARAM)pData->paramFont);
		result = _itow_s(static_cast<int>(round(pData->paramHeight * 0.75f)), buffer, 10);
		if (!result)
			SetWindowText(hwndParamFontSize, buffer);

		SendMessage(hwndList_headers, CB_SELECTSTRING, -1, (LPARAM)pData->headersFont);
...
PWSTR GetFilePath() {
	...
	if (SUCCEEDED(hr))
	{
		...
		hr = SHCreateItemFromParsingName(L"D:\\font\\", NULL, IID_PPV_ARGS(&folderItem));
		if (SUCCEEDED(hr)) {
